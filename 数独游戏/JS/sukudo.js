...
function InitData() {
  ...
  for (j = 0; j < 9; j++)
    for (k = 0; k < 9; k++) {
      bl[j][k] = 0;
      s[j][k] = g.sourcenum.substr(j * 9 + k, 1);
    }
}
/*初始化表格数据----End*/
...
/*辅助函数----Begin*/
function SwapRow() {
  var bigindex = Math.floor(Math.random() * 3),
    index = Math.floor(Math.random() * 3),
    RowA = bigindex * 3 + index,
    RowB = bigindex * 3 + (index + 1) % 3;
  var temp = s[RowA];
  s[RowA] = s[RowB];
  s[RowB] = temp;
}
...
function SwapCol() {
  var bigindex = Math.floor(Math.random() * 3),
    index = Math.floor(Math.random() * 3),
    ColA = bigindex * 3 + index,
    ColB = bigindex * 3 + (index + 1) % 3,
    temp = 0;
  for (li = 0; li < 9; li++) {
    temp = s[ColA][li];
    s[ColA][li] = s[ColB][li];
...
function finit(l) {
  $('#timer').stopTime();
  $('#timer').html('<span>00:00:00<span/>').find('span').css({
    'font-weight': '100'
  });
  InitData();
  CreateSukudoArray();
  ...
  $('.c').hover(function () {
    ...
    $('td').removeClass('sel').unmousewheel();
    $(this).addClass('sel').mousewheel(function (objEvent, intDelta) {
      wc(intDelta, $(this));
    });
    currentmouseint = $(this).num();
  });
  $('#lefts').html('LeftSquares:' + blockscount + '');
  $('#leftsg').refreshnumcount();
  if (l != 0) $('#timer').everyTime(1000, 'timer', function (i) {
    $(this).html('<span>' + f(Math.floor(i / 3600)) + ':' + f(Math.floor(i / 60)) + ':' + f(i % 60) + '</span>').find('span').css({
      'font-weight': '100'
    });
  })
}
...
function wc(intDelta, o) {
  ...
  if (intDelta > 0 && currentmouseint < 9) {
    currentmouseint++;
    o.setwheelnum(currentmouseint);
    o.check();
  } else if (intDelta < 0 && currentmouseint > 1) {
    currentmouseint--;
    o.setwheelnum(currentmouseint);
    o.check();
  } else if ((intDelta > 0 && currentmouseint == 9) || (intDelta < 0 && currentmouseint == 1)) {
    o.setempty();
    o.check();
  } else o.html(o.html());
...
jQuery.fn.extend({
  ...
  clearnum: function (code) {
    if (this.num() == 0) blockscount--;
    else numcount[this.num() - 1]--;
    lastselectednum = this.num();
    if (code > 0 && code < 9) this.text('');
    else this.text('');
    if (s[this.c().x][this.c().y] != this.num()) bl[this.c().x][this.c().y] = 0;
    s[this.c().x][this.c().y] = this.num();
    numcount[this.num() - 1]++;
